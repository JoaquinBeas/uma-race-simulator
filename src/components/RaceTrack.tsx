...
export function TrackSelect(props) {
	const currentCourse = courses[props.courseid] || courses[10101];
	...
	return (
		<div className="trackSelect">
			<select value={trackid} onChange={changeTrack} tabIndex={props.tabindex}>
				{Object.keys(tracknames).map(tid => <option key={tid} value={tid}>{STRINGS.tracknames[tid]}</option>)}
			</select>
			<select value={props.courseid} onChange={changeCourse} tabIndex={props.tabindex + 1}>
				{coursesByTrack[trackid] && coursesByTrack[trackid].map(cid => {
					const c = courses[cid];
					const inout = STRINGS.racetrack[inoutKey[c.course]];
...
function SectionText(props) {
	...
	let text = props.w < 0.085 ? STRINGS.racetrack.short[props.id] : STRINGS.racetrack[props.id];
...
export function RaceTrack(props) {
	...
	const trackNameHeader = useMemo(() => {
		const statStrings = {1: 'Speed', 2: 'Stamina', 3: 'Power', 4: 'Guts', 5: 'Wisdom'};
		const noneStat = 'None';
		const joiner = ', ';
		...
		const inout = STRINGS.racetrack[inoutKey[course.course]];
		...
		return (
			<div className="racetrackHeader">
				<div className="racetrackName">
					{STRINGS.tracknames[course.raceTrackId]}{' '}{desc.replace('{{distance}}', course.distance.toString()).replace('{{inout}}', inout).replace('{{surface}}', surface)}{' '}{STRINGS.racetrack.orientation[course.turn]}
				</div>
	...
	const regions = useMemo(function () {
		return props.regions.reduce((state,desc) => {
			if (desc.type == RegionDisplayType.Immediate && desc.regions.length > 0) {
				...
				const rects = desc.regions.map((r, ri) => {
	...
	return (
		<div className="racetrackWrapper" style={{ width: `${props.width + xOffset + xExtra}px` }}>
			{trackNameHeader}
			<svg version="1.1" width={props.width + xOffset + xExtra} height={props.height + yOffset + yExtra} xmlns="http://www.w3.org/2000/svg" className="racetrackView" data-courseid={props.courseid} onMouseMove={doMouseMove} onMouseEnter={doMouseEnter} onMouseLeave={doMouseLeave}>
