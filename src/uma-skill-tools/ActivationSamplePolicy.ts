...
export const RandomPolicy = Object.freeze({
	sample(regions: RegionList, nsamples: number, rng: PRNG) {
		...
		for (let i = 0; i < nsamples; ++i) {
			...
			const region = regions.find((_,i) => weights[i] > threshold) || regions[0];
			const len = Math.max(region.end - region.start - 10, 0);
			samples.push(region.start + (len > 0 ? rng.uniform(len) : 0));
		}
...
export const StraightRandomPolicy = Object.freeze({
	sample(regions: RegionList, nsamples: number, rng: PRNG) {
		...
		for (let i = 0; i < nsamples; ++i) {
			...
			const len = Math.max(r.end - r.start - 10, 0);
			samples.push(r.start + (len > 0 ? rng.uniform(len) : 0));
		}
		return samples.map(pos => new Region(pos, pos + 10));
	},
...
export const AllCornerRandomPolicy = Object.freeze({
	placeTriggers(regions: RegionList, rng: PRNG) {
		...
		while (triggers.length < 4 && candidates.length > 0) {
			...
			const len = Math.max(c.end - c.start - 10, 0);
			const start = c.start + (len > 0 ? rng.uniform(len) : 0);
		...
		return triggers.length > 0 ? new Region(triggers[0], triggers[0] + 10) : new Region(9999, 9999);  // guaranteed to be the earliest trigger since each trigger is placed after the last one
	},
	sample(regions: RegionList, nsamples: number, rng: PRNG) {
		const samples = [];
		for (let i = 0; i < nsamples; ++i) {
