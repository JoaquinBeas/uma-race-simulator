import React, { createContext, useState, useEffect, useLayoutEffect, useCallback, useMemo, useRef, useContext } from 'react';
...
function o<P extends (string | symbol)[]>(path: (string | symbol | _Lens)[]): Lens<P> {
	...
	return new Proxy(theLens as unknown as any, {
		get(target, prop, _recv) {
			if (typeof prop === 'symbol' || prop === 'toJSON' || prop === 'toString' || prop === 'valueOf' || prop === 'then') {
				return target[prop];
			}
			if (prop == '_iso') {
				...
				if (typeof path[path.length-1] == 'object' && (path[path.length-1] as any).invert != null) {
					return (path[path.length-1] as any).invert();
				} else {
				...
				return function <S,A>(to: (x: S) => A) {
					return o([...path, {
						get(x: any) { return to(x); },
						update(_0: any, _1: any): never { throw new Error('attempted to set a getter'); }
					}]);
				};
			} else if (prop == '_sets') {
				return function <S,T,A,B>(update: (f: (x: A) => B, o: S) => T) {
					return o([...path, {
						get(_: any) { throw new Error('attempted to get a setter'); },
						update
					}]);
				};
			} else if (prop == 'get') {
			...
			} else if (typeof prop === 'string' && +prop === +prop) {
		...
		apply(target, _this, args) { 
			return path.reduce((o,p) => typeof p == 'object' ? p.get(o) : o[p], args[0]); 
		},
...
export function c(...fns: any[]) {
	return (x: any) => fns.reduceRight((x,f) => f(x), x);
}
...
export const State = createContext<any>(null);

export function makeState(getState: () => any) {
	const ref = useRef<any>(null);
	if (ref.current == null) {
		ref.current = {state: getState(), listeners: new Set()};
	}
	const setState = useCallback((newState: any) => {
		ref.current.listeners.forEach((cb: any) => cb(newState));
...
function reactSetter(l: any, ref: any, setState: any) {
	const setter = useMemo(() => new l(), [l]);
	return (f: any) => setState(setter(f, ref.current.state));
}

export function useLens(l: any) {
	const {ref, setState} = useContext(State);
	return [reactGetter(l, ref), reactSetter(l, ref, setState)];
}

export function useGetter(l: any) {
	const {ref} = useContext(State);
	return reactGetter(l, ref);
}

export function useSetter(l: any) {
	const {ref, setState} = useContext(State);
