...
const RaceGraphVisualizer: React.FC<{runData: any, courseDistance: number, courseid: number}> = ({runData, courseDistance, courseid}) => {
	...
	const chartData = useMemo(() => {
		...
		// currentRun.p[0] is position for horse 1, currentRun.v[0] is velocity
		// We want to sample this to not have too many points
		const data: any[] = [];
	...
	return (
		<div className="mt-8 bg-white p-6 rounded-xl shadow-sm border border-slate-200">
			...
			<div className="h-[400px] w-full">
				<ResponsiveContainer width="100%" height="100%">
					<LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
						...
						<XAxis 
							dataKey="distance" 
							label={{ value: 'Distance (m)', position: 'insideBottomRight', offset: -10, fontSize: 12 }} 
							tick={{fontSize: 12}}
						/>
						<YAxis 
							label={{ value: viewMode === 'speed' ? 'Speed (m/s)' : 'HP', angle: -90, position: 'insideLeft', fontSize: 12 }}
							tick={{fontSize: 12}}
			...
			<div className="mt-6">
				<h4 className="text-sm font-bold text-slate-700 mb-2">Track Reference</h4>
				<div className="border rounded-lg overflow-hidden bg-slate-50 p-4 flex justify-center">
					<RaceTrack courseid={courseid} width={600} height={150} regions={[]} />
				</div>
			</div>
		</div>
	);
...
function SimulatorApp() {
	...
	const [activeRightTab, setActiveRightTab] = useState('compare');
		...
		<div className="min-h-screen bg-slate-100 text-slate-800 font-sans p-4 md:p-8">
			<div className="max-w-7xl mx-auto space-y-6">
				...
				<div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
					...
					{/* Left Column: Track & Simulation Controls */}
							...
							<div className="p-4 space-y-4">
								...
								{simulationResult && (
									<div className="mt-6 p-4 bg-slate-50 rounded-lg border border-slate-200 animate-in fade-in slide-in-from-top-2 duration-300">
										<h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider mb-3">Simulation Results</h3>
										<div className="space-y-3">
											<div className="flex items-center justify-between">
												<span className="text-sm text-slate-600">Uma 1 Wins</span>
												<span className="text-lg font-mono font-bold text-blue-600">{simulationResult.wins1}</span>
											</div>
											...
											<div className="flex items-center justify-between">
												<span className="text-sm text-slate-600">Uma 2 Wins</span>
												<span className="text-lg font-mono font-bold text-red-600">{simulationResult.wins2}</span>
											</div>
											<div className="w-full bg-slate-200 rounded-full h-2">
												<div 
											...
											<div className="flex items-center justify-between pt-1">
												...
												<span className="text-sm font-mono font-semibold text-slate-500">{simulationResult.ties}</span>
											</div>
										</div>
										<div className="mt-4 pt-3 border-t border-slate-200 text-center">
											<span className="text-xs text-slate-400">Based on {samples} samples</span>
										</div>
									</div>
								)}
								...
								{simulationResult && (
									<RaceGraphVisualizer 
										...
										courseDistance={CourseHelpers.getCourse(courseid).distance} 
										courseid={courseid}
									/>
								)}
							</div>
